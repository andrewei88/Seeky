...
- Custom classifier: ARYAClassifier.mlpackage (3.2MB, 107 classes). Input: 224x224 RGB. Outputs: softmax probabilities + 1024-dim feature vector
...
### Audio
- Pre-recorded ElevenLabs `.m4a` files at `Vocabulary/{word}/audio.m4a` (107 words)
...
- `VNClassifyImageRequest` for classification (1,303 categories mapped to 107 child words via `label_mappings.json`)
...
- `BundleResourceTests` verifies all 107 audio + timing files are accessible in the built bundle
