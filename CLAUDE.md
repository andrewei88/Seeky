...
- Dual-model classification: VNClassifyImageRequest + MobileCLIP S0 (CoreML). CLIP is primary when available; VN is fallback.
- MobileCLIP S0 image encoder: 22MB `.mlpackage` in Resources, compiled to `.mlmodelc` by Xcode. Input: 256x256 RGB image. Output: 512-dim embedding (`final_emb_1`).
- Pre-computed text embeddings: `text_embeddings.bin` (214KB, 107 words x 512D float32). Generated by `scripts/build_clip_embeddings.py`.
- To regenerate CLIP assets: run `scripts/download_mobileclip.sh` then `scripts/build_clip_embeddings.py`
